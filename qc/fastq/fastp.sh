...
INPUT_PATH=/storage/home/rjb6794/scratch/pen_proj/fastq/
OUTPUT_PATH=/storage/home/rjb6794/scratch/pen_proj/trimmed_fastq_2/
REPORT_PATH=${OUTPUT_PATH}fastp_reports/
...
for file in "${input_files[@]}"; do
  ...
  micromamba run -n bioinfo fastp --thread 16 \
		--in1 "$in1" \
		--in2 "$in2" \
		--out1 "${OUTPUT_PATH}${output_name}_trimmed_R1.fq.gz" \
		--out2 "${OUTPUT_PATH}${output_name}_trimmed_R2.fq.gz" \
		--trim_poly_g \
		--qualified_quality_phred 30 \
		--unqualified_percent_limit 30 \
		--detect_adapter_for_pe \
		--adapter_fasta "${INPUT_PATH}adapters.fa" \
		-j "${REPORT_PATH}${output_name}_fastp.json" \
		-h "${REPORT_PATH}${output_name}_fastp.html"
 done
