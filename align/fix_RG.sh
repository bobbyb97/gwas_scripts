...
BAM_DIR=pen_proj/bam/bwa_mem_imp_v1_Mar24
OUT_DIR=${BAM_DIR}/trimmed_sorted_bam_with_RG
OUT_FILE=${OUT_DIR}/RG.tsv

##--NOTHING BELOW THIS LINE SHOULD BE MODIFIED--##

mkdir -p ${OUT_DIR}

for bam in ${BAM_DIR}/*_trimmed_sorted.bam; do
	samtools view -H ${bam} | grep ^@RG >> ${OUT_FILE}
done
